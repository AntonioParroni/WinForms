    ...
    public partial class Form3 : Form
    {
        ...
        private void FormMouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                {
                    MessageBox.Show("Невозможно создать «статик» меньше чем 10х10", "Ошибка!", MessageBoxButtons.OK,
            ...
            {
                MessageBox.Show("Для создания «статика» нажмите левую кнопку", "Ошибка!", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
        }
        ...
        private void LabelMouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                foreach (Label item in Controls)
                {
                    ...
                    {
                        Text =
                            $"«Статик» номер №{item.Text}, Площадь {item.Width * item.Height}, Координаты Х = {item.Location.X} Y = {item.Location.Y}";
                    }
                }
            }
        }
